  ...
  return (
    <div className="h-screen bg-black text-white flex flex-col">
      ...
      <header className="px-6 md:px-12 py-5 flex-shrink-0 flex items-center justify-between">
        <span className="text-2xl font-black tracking-tight">BRIGHTMIND</span>
      ...
      <div className="flex-1 flex items-center justify-center overflow-hidden">
        ...
        {step === 0 && (
          <motion.div key="s0" {...fade} className="w-full max-w-md px-6 text-center">
            <h1 className="text-2xl font-black mb-2">როგორ გინდა გამოიყენო BrightMind?</h1>
                ...
                return (
                  <button
                    ...
                    className={`p-6 rounded border text-center transition-colors duration-200 ${
                      selected ? 'border-[#E50914] bg-[#E50914]/10' : 'border-white/10 bg-white/[0.02] hover:border-white/20'
                    }`}
                  >
                    <div className={`w-12 h-12 rounded-full mx-auto mb-4 flex items-center justify-center ${selected ? 'bg-[#E50914]' : 'bg-white/[0.06]'}`}>
                    ...
                    {selected && (
                      <div className="mt-3 flex justify-center">
                        <div className="w-5 h-5 bg-[#E50914] rounded-full flex items-center justify-center">
        ...
        {step === 1 && (
          <motion.div key="s1" {...fade} className="w-full max-w-sm px-6 text-center">
            ...
            <input
              ...
              placeholder="სახელი"
              className="w-full px-5 py-4 bg-white/[0.04] border border-white/10 rounded text-white text-center text-lg placeholder-white/20 focus:outline-none focus:border-[#E50914] transition-colors"
            />
            ...
            <div className="mt-8">
              <button
                ...
                className={`px-10 py-3 rounded text-sm font-bold transition-colors ${
                  name.trim() ? 'bg-[#E50914] text-white hover:bg-[#c70812]' : 'bg-white/[0.04] text-white/20 cursor-not-allowed'
                }`}
              >
                გაგრძელება
              </button>
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 py-4 px-3 rounded border transition-colors duration-200 ${
                      selected ? 'border-[#E50914] bg-[#E50914]/10' : 'border-white/[0.08] bg-white/[0.02] hover:border-white/20'
                    }`}
                  >
                    <CategoryIcon iconName={cat.icon} className={`w-5 h-5 ${selected ? 'text-[#E50914]' : 'text-white/30'}`} />
        ...
        {step === 3 && (
          <motion.div key="s3" {...fade} className="w-full max-w-md px-6 text-center">
            <div className="w-14 h-14 bg-[#E50914] rounded-full flex items-center justify-center mx-auto mb-6">
            ...
            <button
              ...
              className="px-10 py-3 bg-[#E50914] text-white rounded text-sm font-bold hover:bg-[#c70812] transition-colors">
        ...
        <div className="pb-8 flex justify-center gap-2">
          {Array.from({ length: totalSteps }).map((_, i) => (
            <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${
              i === step ? 'w-6 bg-[#E50914]' : i < step ? 'w-1.5 bg-white/20' : 'w-1.5 bg-white/[0.06]'
            }`} />
          ))}
        </div>
      )}
