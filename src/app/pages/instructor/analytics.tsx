    ...
    <div className="min-h-screen bg-background pt-32 pb-20 px-4 md:px-12">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <h1 className="text-2xl font-black text-foreground">ანალიტიკა</h1>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
              <div className="bg-surface border border-border-subtle rounded p-5">
                ...
                <p className="text-foreground text-2xl font-black">{stats?.totalRevenue ?? 0} ₾</p>
                <p className="text-foreground-subtle text-xs mt-1">ჯამური შემოსავალი</p>
              </div>
              <div className="bg-surface border border-border-subtle rounded p-5">
                <Users className="w-5 h-5 text-blue-400 mb-2" />
                <p className="text-foreground text-2xl font-black">{stats?.totalStudents ?? 0}</p>
                <p className="text-foreground-subtle text-xs mt-1">სულ სტუდენტები</p>
              </div>
              <div className="bg-surface border border-border-subtle rounded p-5">
                <Star className="w-5 h-5 text-yellow-400 mb-2" />
                <p className="text-foreground text-2xl font-black">{stats?.avgRating?.toFixed(1) ?? '0.0'}</p>
                <p className="text-foreground-subtle text-xs mt-1">საშუალო რეიტინგი</p>
              </div>
              <div className="bg-surface border border-border-subtle rounded p-5">
                <TrendingUp className="w-5 h-5 text-brand mb-2" />
                <p className="text-foreground text-2xl font-black">{stats?.recentEnrollments ?? 0}</p>
                <p className="text-foreground-subtle text-xs mt-1">ბოლო 30 დღე</p>
              </div>
            </div>

            {/* Revenue split info */}
            <div className="bg-surface border border-border-subtle rounded p-5 mb-10">
              <h3 className="text-foreground font-bold text-sm mb-3">შემოსავლის განაწილება</h3>
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-foreground-subtle">ინსტრუქტორი (60%)</span>
                ...
                <div className="flex-1">
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-foreground-subtle">პლატფორმა (40%)</span>
                    <span className="text-foreground-subtle font-bold">{Math.round((stats?.totalRevenue ?? 0) * 0.4)} ₾</span>
                  </div>
                  <div className="h-2 bg-surface-raised rounded-full overflow-hidden">
                    <div className="h-full bg-surface-active rounded-full" style={{ width: '40%' }} />
                  </div>
                </div>
              </div>
              <p className="text-foreground-faint text-[10px] mt-3">30-დღიანი hold პერიოდი გამოტანამდე</p>
            </div>

            {/* Per-course stats */}
            <h2 className="text-lg font-bold text-foreground mb-4">კურსების სტატისტიკა</h2>
            <div className="space-y-2">
              {courses.map((course: CourseFromApi) => {
                const c = apiCourseToCourse(course);
                return (
                  <div key={c.id} className="flex items-center gap-4 bg-surface border border-border-subtle rounded p-4 hover:bg-surface-raised transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-foreground-faint text-xs">{Number(course.price).toFixed(0)} ₾</p>
                    </div>
