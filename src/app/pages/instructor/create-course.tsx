...
export function CreateCourse() {
  ...
  const steps = [
    ...
    <div key="0" className="space-y-6">
      <div>
        <label className="block text-xs text-foreground-subtle mb-2 font-medium">კურსის სახელი *</label>
      ...
      <div>
        <label className="block text-xs text-foreground-subtle mb-2 font-medium">ქვესათაური</label>
        <input type="text" value={form.subtitle} onChange={e => updateForm('subtitle', e.target.value)}
          placeholder="მოკლე აღწერა"
          className="w-full px-4 py-3 bg-surface-raised border border-border-muted rounded text-foreground text-sm placeholder-foreground-faint focus:outline-none focus:border-brand" />
      </div>
      <div>
        <label className="block text-xs text-foreground-subtle mb-2 font-medium">აღწერა</label>
    ...
    <div key="1" className="space-y-6">
      <div>
        <label className="block text-xs text-foreground-subtle mb-3 font-medium">კატეგორია *</label>
            ...
            <button key={cat.id} onClick={() => toggleCategory(cat.id)}
              className={`px-4 py-2 rounded border text-sm transition-all active:scale-[0.97] ${
                form.categoryIds.includes(cat.id)
                  ...
                  : 'bg-transparent border-border-muted text-foreground-subtle hover:border-border'
              }`}>
              {cat.name}
            </button>
          ))}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs text-foreground-subtle mb-2 font-medium">დონე</label>
          <select value={form.level} onChange={e => updateForm('level', e.target.value)}
            className="w-full px-4 py-3 bg-surface-raised border border-border-muted rounded text-foreground text-sm focus:outline-none focus:border-brand">
        ...
        <div>
          <label className="block text-xs text-foreground-subtle mb-2 font-medium">ფასი (₾) *</label>
    ...
    <div key="2" className="space-y-6">
      <div>
        <label className="block text-xs text-foreground-subtle mb-2 font-medium">თამბნეილი (URL)</label>
        ...
        {!form.thumbnailUrl && (
          <div className="mt-4 aspect-video rounded border-2 border-dashed border-border-muted flex flex-col items-center justify-center">
            <Image className="w-8 h-8 text-foreground-faint mb-2" />
            <p className="text-foreground-faint text-xs">თამბნეილის URL შეიყვანეთ</p>
          </div>
        )}
      </div>

      {/* Summary */}
      <div className="bg-surface border border-border-subtle rounded p-5 space-y-3">
        <h3 className="text-foreground font-bold text-sm">შეჯამება</h3>
        <div className="text-xs text-foreground-subtle space-y-1">
          <p><span className="text-foreground-secondary">სახელი:</span> {form.title || '—'}</p>
          <p><span className="text-foreground-secondary">ფასი:</span> {form.price} ₾</p>
          <p><span className="text-foreground-secondary">დონე:</span> {form.level}</p>
          <p><span className="text-foreground-secondary">კატეგორიები:</span> {form.categoryIds.length || '—'}</p>
        </div>
      </div>
    </div>,
  ];
    ...
    <div className="min-h-screen bg-background pt-32 pb-20 px-4 md:px-12">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-black text-foreground">კურსის შექმნა</h1>
            <p className="text-foreground-subtle text-xs mt-0.5">ნაბიჯი {step + 1}/3</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-8">
          {[0, 1, 2].map(i => (
            <div key={i} className="flex-1 h-[3px] rounded-full overflow-hidden bg-surface-raised">
              <div className={`h-full rounded-full transition-all duration-500 ${i <= step ? 'bg-brand w-full' : 'w-0'}`} />
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-between mt-8">
          <button onClick={() => step > 0 ? setStep(step - 1) : navigate('/instructor')}
            className="px-6 py-2.5 text-foreground-secondary text-sm font-medium hover:text-foreground transition-colors">
            {step > 0 ? 'უკან' : 'გაუქმება'}
          </button>
          {step < 2 ? (
            <button onClick={() => setStep(step + 1)} disabled={!canProceed}
              className={`px-8 py-2.5 rounded-full text-sm font-bold transition-all active:scale-95 ${
                canProceed ? 'bg-brand text-white hover:bg-brand-hover' : 'bg-surface text-foreground-faint cursor-not-allowed'
              }`}>
              შემდეგი
            </button>
          ) : (
